...
def main():
  ...
  # add_accuracy_meter(image,tells)
    ...
    with mp.solutions.hands.Hands(
      ...
      if len(args.input) == 4:
        ...
        if isinstance(INPUT, str) and INPUT.find('.') > -1: # from file
          ...
          cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1980)  # 4K Width
          cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 1080)  # 4K Height
...
def add_truth_meter(image, tell_count):
  # Calculate number of deceptive and truthful tells
  deceptive_tells = 0
  truthful_tells = 0
    
  for tell in tells.values():
        if tell['text'] in ['Increased blinking', 'Hand covering face', 'Lip compression']:
            deceptive_tells += 1
        else:
            truthful_tells += 1
    
    # Calculate accuracy as a percentage
  total_tells = truthful_tells + deceptive_tells
  if total_tells > 0:
    accuracy = (truthful_tells / total_tells) * 100
  else:
    accuracy = 100  # default to 100% if no tells are present

    # Add accuracy bar at the bottom of the image
  width = image.shape[1]
  accuracy_width = int((accuracy / 100) * width)  # Scale width based on accuracy percentage
  accuracy_bar_color = (0, 255, 0) if accuracy > 50 else (0, 0, 255)  # Green for truthful, red for deceptive

    # Draw accuracy meter (green for high accuracy, red for low)
  cv2.rectangle(image, (0, image.shape[0] - 30), (accuracy_width, image.shape[0]), accuracy_bar_color, -1)
  cv2.putText(image, f"Truthfulness: {accuracy:.2f}%", (10, image.shape[0] - 10), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
  #width = image.shape[1]
  #sm = int(width / 64)
  #bg = int(width / 3.2)
...
'''
#Caluculate accurcy
def add_accuracy_meter(image, tells):
    # Calculate number of deceptive and truthful tells
    #deceptive_tells = 0
    truthful_tells = 0
    ...
    #cv2.rectangle(image, (0, image.shape[0] - 30), (accuracy_width, image.shape[0]), accuracy_bar_color, -1)
    #cv2.putText(image, f"Truthfulness: {accuracy:.2f}%", (10, image.shape[0] - 10), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
    #print(image, f"Truthfulness: {accuracy:.2f}%", (10, image.shape[0] - 10)) #, cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)

    #cv2.putText(image ,f"Truthfulness: {accuracy:.2f}%", (10,50),cv2.FONT_HERSHERY_SIPLME, 1,(0,255,0), 2)
    #cv2.imshow("Accuracy",image)
    print(f"Truthfulness: {accuracy:.2f}%")
    #if cv2.waitKey(1) & 0xFF == ord('q'):
